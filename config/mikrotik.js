...
// Fungsi untuk generate voucher hotspot secara massal
async function generateHotspotVouchers({ quantity, length, profile, type, charType }) {
    const conn = await getMikrotikConnection();
    if (!conn) {
        throw new Error('Koneksi ke Mikrotik gagal');
    }

    const createdVouchers = [];
    const randomString = (charLength) => {
        const chars = charType === 'numeric' 
            ? '0123456789' 
    ...
    for (let i = 0; i < quantity; i++) {
        const username = randomString(length);
        // Tentukan password berdasarkan tipe: 'voucher' (sama) atau 'member' (berbeda)
        const password = (type === 'voucher') ? username : randomString(length);
        ...
        try {
            await conn.write('/ip/hotspot/user/add', [
                ...
                `=password=${password}`,
                `=profile=${profile}`,
                `=comment=Generated by wa-admin`
            ]);
            createdVouchers.push({ username, password, profile });
        } catch (error) {
            logger.error(`Gagal membuat voucher ${username}: ${error.message}`);
            // Lanjutkan jika satu gagal, jangan hentikan seluruh proses
        }
    }

    if (createdVouchers.length === 0 && quantity > 0) {
        throw new Error('Tidak ada voucher yang berhasil dibuat. Cek koneksi atau log Mikrotik.');
    }
